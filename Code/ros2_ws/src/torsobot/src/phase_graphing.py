...
save_file_name = csv_file_name.split(".")[0] + "_phase.png"
save_file_path = dir_path / "graphs" / "phase_plots" / save_file_name
...
sc = axs.scatter(wheel_pos, wheel_vel, c=timestamp, cmap="viridis", edgecolor="none")
...
# Optional: Add "Minor" ticks (smaller lines between the main ones)
axs.yaxis.set_minor_locator(MultipleLocator(0.25))
axs.xaxis.set_minor_locator(MultipleLocator(0.025))

axs.set_ylim((-2, 4))

axs.tick_params(axis="y", which="minor", length=4, color="r")

axs.axvline(-np.pi / 10)
axs.axvline(np.pi / 10)
