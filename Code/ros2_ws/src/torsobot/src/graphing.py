...
# Subplot 2
axs[0].plot(timestamp, wheel_torque, label="wheel_torque")
axs[0].plot(timestamp, wheel_cmd_torque, color="red", label="wheel_cmd_torque")
axs[0].minorticks_on()
axs[0].grid(which="both")
# axs[1].set_xlabel("time (ms)")
axs[0].set_ylabel("torque (N.m)")
axs[0].set_ylim([min_trq_lim, max_trq_lim])
axs[0].legend()
axs[0].set_title(plot_text, fontsize=10)

# Subplot 1
axs[1].plot(
    timestamp, desired_torso_pitch_deg, color="red", label="desired torso pitch"
)
axs[1].plot(timestamp, torso_pitch, label="actual torso pitch")

# draw horizontal line for pitch
# plt.axhline(desired_torso_pitch_deg)
...
# Subplot 3
axs[2].plot(timestamp, torso_pitch_rate, label="torso_pitch_rate")
axs[2].minorticks_on()
