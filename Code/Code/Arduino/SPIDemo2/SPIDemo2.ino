#include <SPI.h>
#include <SPISlave.h>

SPISettings spisettings(1000 * 1000, MSBFIRST, SPI_MODE0);

volatile bool recvBuffReady = false;
const int byte_len = 1;  // Changed to 4 to match python code.
char recvByte[byte_len];

const int data_len = 16;  // Changed to 4 to match python code.
char recvBuff[data_len];

int recvIdx = 0;
char recvFirstByte = ' ';

void recvCallback(uint8_t *data, size_t len) {
  memcpy(recvBuff + recvIdx, data, byte_len);
  // if (recvIdx
  //     == 0) {
  //   recvFirstByte = recvBuff[0];
  // }
  recvIdx += byte_len;

  if (recvIdx >= data_len) {
    recvBuffReady = true;
    recvIdx = 0;
  }
}

int sendcbs = 0;
char sendByte[byte_len];

char sendBuff[data_len] = " ";
char sendBuff1[data_len] = "exams are borin";
char sendBuff2[data_len] = "aalloo is goods";
int i = 0;

void sentCallback() {
  memset(sendBuff, 0, sizeof(sendBuff));
  memset(sendByte, 0, sizeof(sendByte));
  i = sendcbs++ % data_len;
  if (recvBuff[0] == '1') {
    memcpy(sendBuff, sendBuff1, data_len);
  } else {
    memcpy(sendBuff, sendBuff2, data_len);
  }
  sprintf(sendByte, &sendBuff[i]);

  // sprintf(sendByte, &sendBuff[i]);
  SPISlave1.setData((uint8_t *)sendByte, sizeof(sendByte));
  // Serial.print("Sending data: ");
  // Serial.println(sendByte);
}

void setup() {
  Serial.begin(115200);

  SPISlave1.setSCK(10);
  SPISlave1.setTX(11);
  SPISlave1.setRX(12);
  SPISlave1.setCS(13);
  sentCallback();
  SPISlave1.onDataRecv(recvCallback);
  SPISlave1.onDataSent(sentCallback);
  SPISlave1.begin(spisettings);
  delay(3000);
  Serial.println("S-INFO: SPISlave started");
}

void loop() {
  if (recvBuffReady) {
    Serial.print("Received: ");
    Serial.print(recvBuff);
    Serial.println();
    memset(recvBuff, 0, sizeof(recvBuff));  // clear the buffer.
    i = 0;
    recvBuffReady = false;
  }
}