...
bool torsobot_interfaces__msg__data__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // motor_acc
    PyObject * field = PyObject_GetAttrString(_pymsg, "motor_acc");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->motor_acc = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * torsobot_interfaces__msg__data__convert_to_py(void * raw_ros_message)
{
  ...
  {  // motor_acc
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->motor_acc);
    {
      int rc = PyObject_SetAttrString(_pymessage, "motor_acc", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
