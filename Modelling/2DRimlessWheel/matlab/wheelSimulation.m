function [y_sol, sol, event_sol, frame]= wheelSimulation(slope_angle, l, m, I, n, init_con, stop_vel, time_interval, solver_type, solver_max_step, phase_plot, fig_plot, A)
...
% create ode object
F = ode(ODEFcn = @diffFunc, InitialValue = init_con, EventDefinition = E, Solver= solver_type, Parameters=[slope_angle, n, collision_angle, vel_coeff, stop_vel, A]);
% set solver options
F.SolverOptions.MaxStep = solver_max_step;
F.RelativeTolerance = 1e-6; % Tighten from default 1e-3
F.AbsoluteTolerance = 1e-6; % Tighten from default 1e-6
