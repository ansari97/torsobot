...
theta_dot_before_simple = zeros(1, length(phi_des));

for i=1:length(phi_des)

%% Simplified analyses assuming that torso has zero vel pre- and post-collision
h = Iw + (m+M)*L^2 + m*l*L*cos(theta_c + phi_des(i));
j = Iw + (m+M)*L^2*cos(2*theta_c) + m*l*L*cos(theta_c - phi_des(i));

f = j./h;

actuator_work = 2*theta_c*m*grav*l*sin(phi_des(i) + slope_angle);
...
syms theta_dot_before real

theta_dot_after = f*theta_dot_before;
...
actuator_impulse_work = -torque_impulse/2 * (theta_dot_after+theta_dot_before);
%
...
KE = C*(1-f.^2)*theta_dot_before;
%

% energy balance
actuator_total_work = actuator_impulse_work + actuator_work;

energy_balance = actuator_total_work == KE;

theta_dot_before_ans = double(solve(energy_balance, theta_dot_before));

if theta_dot_before_ans
    if theta_dot_before_ans(1)>0
        theta_dot_before_simple(i) = theta_dot_before_ans(1);
    elseif theta_dot_before_ans(2)>0
        theta_dot_before_simple(i) = theta_dot_before_ans(2);
    end
else
    theta_dot_before_simple(i) = -1;
end
...
% COT analysis
E_loss = total_work;
COT_approx_simple = E_loss/COT_denominator;
